...
train_data = loadtxt("meta_nn_ready_audience.csv", delimiter = ',', skiprows=1)
...
prediction_df = pd.DataFrame(prediction, columns = ["predicted_aud"])

## read in test data as dataframe to compare to
correct_df = pd.DataFrame(yt, columns = ["aud"])
...
comparison_df["percent_diff"] = (100 * abs(comparison_df["aud"] -
                                                comparison_df["predicted_aud"])
                                                / comparison_df["aud"])

comparison_df.to_csv("meta_predicted_aud.csv", index=False)